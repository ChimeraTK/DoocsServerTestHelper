...
extern "C" int nanosleep(__const struct timespec *__requested_time,
  ...
  static thread_local std::unique_lock<std::mutex> update_lock{
      DoocsServerTestHelper::update_mutex};
  do {
    update_lock.unlock();
    usleep(1);
    update_lock.lock();
...
extern "C" int sigwait(__const sigset_t *__restrict __set,
  ...
  static thread_local std::unique_lock<std::mutex> sigusr1_lock{
      DoocsServerTestHelper::sigusr1_mutex};
  do {
    sigusr1_lock.unlock();
    usleep(1);
    sigusr1_lock.lock();
...
std::unique_lock<std::mutex>
    DoocsServerTestHelper::update_lock(DoocsServerTestHelper::update_mutex);
std::unique_lock<std::mutex>
    DoocsServerTestHelper::sigusr1_lock(DoocsServerTestHelper::sigusr1_mutex);

/**********************************************************************************************************************/

void DoocsServerTestHelper::setDoNotProcessSignalsInDoocs(
    bool _doNotProcessSignalsInDoocs) {
...
void DoocsServerTestHelper::doocsSetSpectrum(const std::string &name,
  ...
  spectrum.d_spect_array.d_spect_array_val =
      (float *)value.data(); // casts const-ness away, but will not be modified
                             // (hopefully)
  ed.set(&spectrum);
  ...
  // check for error
  ASSERT(res.error() == 0,
         std::string("Error writing spectrum property ") + name);
}

void DoocsServerTestHelper::doocsSetIIII(const std::string &name,
