 ...
 *
 *  Skeleton header file for a sequence of tests which have to be in separate executables but share most of its code.
 */
...
// flag whether we are done with testing. Must be global, since we need to access it from the SIGINT handler
...
class HelperTest {
  public:
    ...
    // routines running in separate threads each with a loop simulating the sigusr1/update triggering behaviour of DOOCS
    void sigusr1();
    void update();

    // helper routines to allow the threads running sigwait resp. nanosleep one (more) time
    void allowSigusr1();
    void allowUpdate();

    // helper routines to wait until the threads have completed their sigwait resp. nanosleep
    void waitSigusr1();
    void waitUpdate();
    ...
    // only used from the test routines thread: the last promises sent to the queue (as futures)
...
void HelperTest::update() {
  struct timespec magicSleepTime{DoocsServerTestHelper::magic_sleep_time_sec,
...
void HelperTest::testRoutine() {
  ...
  // block sigusr1 for this process and thread (otherwise the signal would terminate the process)
  sigset_t set;
  sigemptyset(&set);
  sigaddset(&set, SIGUSR1);
  ...
  // initialise the future queues (only those which are being filled from this thread)
